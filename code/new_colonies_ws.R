...
nb.weather <- subset(weather, Location == 'NB' & Year == 1999)
write.csv(nb.weather, 'data/nb_real_weather.csv', row.names = FALSE)
...
age.fun <- function(Prov, year) {
  prov <- tolower(Prov)
  yr <- substr(as.character(year), 3, 4)
  
  wthr <- subset(weather, Location == Prov & Year == year)
  write.csv(wthr, paste0('data/', prov, yr, '_real_weather.csv'))
