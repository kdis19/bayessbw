...
data.lst <- parLapply(cl,1:N, function(x) {
  set.seed(x + 1)
  gp <- gen.pops(1)[[1]]
...
names(all.data) <- c('temp1', 'temp2', 'stage', 'time1', 'time2',
                     'nobs', 'cup', 'prior.samp')
...
#write.csv(all.data, 'data/noss_sim_data_reduced.csv', row.names = FALSE)
#write.csv(priors.df, 'data/noss_sim_priors_reduced.csv', row.names = FALSE)
...
gr.lst1 <- parLapply(cl1, 1:N, function(i) {
...
write.csv(diag.df, 'code/diagnostics_reduced.csv')
w <- which(diag.df$divergences == 0)

post.lst <- lapply(gr.lst1, as.data.frame)[w]
B <- round(length(w)/20)
L <- B*2
post.lst.thin <- lapply(1:length(post.lst), function(x) {
  dat <- post.lst[[x]]
  thinned <- dat[seq(1, nrow(dat), length.out = L),]
  dat$prior.samp <- w[x]
  return(dat)
})

##### Analyse SBC for curve parameters #####
curve.pars <- c('HA', 'TL', 'HL', 'TH', 'HH')
priors.df.new <- priors.df[w,]
...
coverage <- apply(ranks.df, 2, function(x) {length(which(x/L > 0.05 & x/L < 0.95))/length(x)})
write.csv(ranks.df, 'code/ranks_reduced.csv')

qbn <- qbinom(c(0.005, 0.5, 0.995), length(w), 1/B)

par(mfrow = c(3, 3))
for (i in 1:ncol(ranks.df)) {
  hist(as.data.frame(ranks.df)[,i], main = names(ranks.df)[i], breaks = B - 1)
  abline(h = qbn[1], col = 'red')
  abline(h = qbn[2])
  abline(h = qbn[3], col = 'red')
}

##### Analyse SBC for development rates #####
...
rate.lst <- lapply(1:length(post.lst.thin), function(x) {
  post.df <- post.lst.thin[[x]]
  ...
  p.lst <- lapply(1:5, function(ind) {
    ...
    names(pars) <- c(cnms, 'rho', 's_eps', 's_upsilon')
    pars$HL <- -abs(pars$HL)
...
coverage.rates <- apply(r.df, 2, function(x) 
  {length(which(x/L > 0.05 & x/L < 0.95))/length(x)})
...
m.rdf$stage <- sapply(as.character(m.rdf$variable), 
                      function(x) {strsplit(x, split = '_')[[1]][2]})

ggplot() +
  geom_hline(yintercept = seq(qbn[1], qbn[3], by = 0.1), alpha = 0.5, col = 'lightgrey') +
  geom_histogram(data = m.rdf, aes(x = value), bins = B, col = 'black') +
  facet_wrap(vars(variable), scales = 'free', nrow = 7, ncol = 5) +
  theme_minimal() 

##### Power transformation parameters for \sigma_{\epsilon} #####
post.lst.ind <- lapply(1:length(post.lst), function(i) {
  x <- post.lst[[i]]
  x$ind <- w[i]
  return(x)
})
post.lst.all <- bind_rows(post.lst.ind)
