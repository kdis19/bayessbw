...
px[0] = DW * py[0];
  ...
  Type objective_function<Type>::operator() ()
  {
    ...
    DATA_IVECTOR(eps_block);
    DATA_IVECTOR(ups_block);
    DATA_VECTOR(k_vec);
    ...
    PARAMETER_VECTOR(HA_mult);
    PARAMETER_VECTOR(HL_mult);
    PARAMETER_VECTOR(HH_mult);
    PARAMETER_VECTOR(TH_mult);
    PARAMETER_VECTOR(TL_mult);
    PARAMETER_VECTOR(rho_mult);
    ...
    for (int i=0; i<time1.size(); i++) {
      Type srho = rho(stage(i));
      srho *= rho_mult(eps_block(i));
      
      Type sHL = HL*HL_mult(colony(i));
      Type sHA = HA*HA_mult(colony(i));
      Type sHH = HH*HH_mult(colony(i));
      
      Type sTL = TL*TL_mult(colony(i));
      Type sTH = TH*TH_mult(colony(i));
      
      TA = calc_TA(-sHL, sHH, sTL, sTH);
      
      // Calculate dev times for both sustainable and lethal temps
      tpred1 = calc_pred(temp1(i), srho, sHA, sTL, -sHL, sTH, sHH, TA);
      tpred2 = calc_pred(temp2(i), srho, sHA, sTL, -sHL, sTH, sHH, TA);
      
      // c_upsilon1 = exp(upsilon(t_block1(i))*s_upsilon(stage(i)));
      // c_upsilon2 = exp(upsilon(t_block2(i))*s_upsilon(stage(i)));
      
      c_upsilon1 = exp(upsilon(t_block1(i))*s_upsilon(ups_block(i)));
      c_upsilon2 = exp(upsilon(t_block2(i))*s_upsilon(ups_block(i)));
      ...
      epsm1_std = epsm1/s_eps(eps_block(i));
      epsij_std = epsij/s_eps(eps_block(i));
    ...
    if (use_prior == 1) {
      ...
      jnll -= dnorm(TH, Type(305), Type(2), 1);
      
      jnll -= sum(dnorm(log(s_eps), Type(-1.5), Type(0.1), 1));
      // jnll -= dnorm(log(s_upsilon), Type(-2.5), Type(0.05), 1);
      // jnll -= dnorm(log(s_upsilon), Type(-3.5), Type(0.05), 1);
      jnll -= sum(dnorm(log(s_upsilon), Type(-2.5), Type(0.1), 1));
      
      jnll -= sum(dnorm(log(HL_mult), Type(0), Type(0.15), 1));
      jnll -= sum(dnorm(log(HA_mult), Type(0), Type(0.15), 1));
      jnll -= sum(dnorm(log(HH_mult), Type(0), Type(0.15), 1));
      jnll -= sum(dnorm(log(TL_mult), Type(0), Type(0.005), 1));
      jnll -= sum(dnorm(log(TH_mult), Type(0), Type(0.005), 1));
      jnll -= sum(dnorm(log(rho_mult), Type(0), Type(0.15), 1));
      
      // jnll -= sum(dnorm(log(HL_mult), Type(0), Type(0.4), 1));
      // jnll -= sum(dnorm(log(HA_mult), Type(0), Type(0.3), 1));
      // jnll -= sum(dnorm(log(HH_mult), Type(0), Type(0.3), 1));
      // jnll -= sum(dnorm(log(TL_mult), Type(0), Type(0.01), 1));
      // jnll -= sum(dnorm(log(TH_mult), Type(0), Type(0.01), 1));
      // jnll -= sum(dnorm(log(rho_mult), Type(0), Type(0.3), 1));
